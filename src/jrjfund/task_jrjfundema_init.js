...
const { TASK_NAMEID_JRJFUND_EMA_INIT } = require('../taskdef');
...
class TaskJRJFundEMA_Init extends Task {
    constructor(taskfactory, cfg) {
        super(taskfactory, TASK_NAMEID_JRJFUND_EMA_INIT, cfg);
    }
    ...
    countEMA(lst, mai, bi, bj) {
        let p = 2 / (mai + 1);
        if (bj == mai - 1) {
            return p * lst[bi - bj].accum_net;
        }

        return p * lst[bi - bj].accum_net + (1 - p) * this.countEMA(lst, mai, bi, bj + 1);
    }
    ...
    async procFormat(ti, code) {
        ...
        if (rows.length > 0) {
            ...
            await FinanceMgr.singleton.saveJRJFundFactor2('jrjfundema_' + ti, lst, 512);
    ...
    onStart() {
        ...
        MysqlMgr.singleton.start().then(async () => {
            ...
            for (let ii = 0; ii < 10; ++ii) {
                let per = ii / 10;
                await FinanceMgr.singleton.createFundFactor('jrjfundema_' + ii, 'ema', 2, 50);
...
taskFactory.regTask(TASK_NAMEID_JRJFUND_EMA_INIT, (taskfactory, cfg) => {
    return new TaskJRJFundEMA_Init(taskfactory, cfg);
});

exports.TaskJRJFundEMA_Init = TaskJRJFundEMA_Init;