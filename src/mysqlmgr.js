...
class MysqlMgr {
    ...
    async start() {
        for (let key in this.mapCfg) {
            try {
                ...
                log('info', 'MysqlMgr.start() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' ok!');
            }
            catch (err) {
                log('error', 'MysqlMgr.start() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
            }
        }
    }
    ...
    async onError(key, err) {
        log('error', 'MysqlMgr.onError() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
        if (err.code == 'ETIMEDOUT' || err.code == 'ECONNRESET' || err.code == 'ECONNREFUSED' || err.code == 'PROTOCOL_CONNECTION_LOST' ||
            ...
            try {
                ...
                log('error', 'MysqlMgr.onError() reconnect failed. ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
            }
